...
B=(h,p)=>{try{const s=C({hostname:h,port:p,keepAlive:true});return s.opened.then(()=>s).catch(()=>null)}catch{return null}};
let m=0,l=0
...
const F=()=>{!t&&l<12&&s.buffer.byteLength===s.length&&s.buffer.byteLength<=4096&&(P[l++]=s)}
for(let i=n;i--;)s[i]=d.charCodeAt(i)
for(let i=16;i--;)if(s[i+1]^U[i]){F();return new Response(null,{status:400})}
const[x,p,z,v]=A(s),k=await B(x,p)||await B('sjc.o00o.ooo',443);if(!k){F();return new Response(null,{status:502})}
